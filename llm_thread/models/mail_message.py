...
class MailMessage(models.Model):
    ...
    @api.model
    def _message_fetch(
        self, domain, search_term=None, before=None, after=None, around=None, limit=30, **kwargs
    ):
        ...
        return super()._message_fetch(domain, search_term=search_term, before=before, after=after, around=around, limit=limit, **kwargs)
    ...
    def _extras_to_store(self, store, format_reply):
        ...
        for message in self:
            ...
            if data:  # Only add to store if we have data
                store.add_model_values(message._name, {"id": message.id, **data})
