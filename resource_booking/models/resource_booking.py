...
# Copyright 2025 SGC Tech AI - AI-enhanced scheduling
...
class ResourceBooking(models.Model):
    ...
    ai_suggestion = fields.Text(
        string="AI Scheduling Suggestion",
        compute="_compute_ai_suggestion",
        help="AI-generated scheduling suggestion based on availability patterns and conflicts.",
    )
    ai_confidence = fields.Float(
        string="AI Confidence",
        compute="_compute_ai_suggestion",
        help="AI confidence score (0-100) for the scheduling suggestion.",
    )
    ai_narrative = fields.Html(
        string="AI Booking Summary",
        compute="_compute_ai_narrative",
        help="AI-generated narrative summary of this booking.",
    )
    ...
    def _compute_ai_suggestion(self):
        for booking in self:
            if not booking.start or not booking.type_id:
                booking.ai_suggestion = False
                booking.ai_confidence = 0.0
                continue
            best_combination = booking._get_best_combination()
            if best_combination:
                ...
                available = booking._get_intervals(
                    start_dt, end_dt, best_combination
                )
                total_slots = len(available._items) if available._items else 0
                booking.ai_confidence = min(100.0, total_slots * 15.0)
                resource_names = ", ".join(
                    best_combination.resource_ids.mapped("name")
                )
                booking.ai_suggestion = (
                    f"Recommended: {resource_names or 'Auto-assigned'} "
                    f"| {total_slots} available slot(s) | "
                    f"Confidence: {booking.ai_confidence:.0f}%"
                )
            else:
                booking.ai_suggestion = "No suitable resource combination found."
                booking.ai_confidence = 0.0

    def _compute_ai_narrative(self):
        for booking in self:
            if not booking.id:
                booking.ai_narrative = False
                continue
            parts = []
            state_labels = {
                "pending": "⏳ Pending",
            ...
            state_label = state_labels.get(booking.state, booking.state)
            parts.append(f"<strong>Status:</strong> {state_label}")
            if booking.type_id:
                parts.append(f"<strong>Type:</strong> {booking.type_id.display_name}")
            if booking.combination_id:
                parts.append(
                    f"<strong>Resources:</strong> "
                    f"{booking.combination_id.resource_ids.mapped('name')}"
                )
            if booking.start:
                parts.append(
                    f"<strong>Start:</strong> {booking.start.strftime('%Y-%m-%d %H:%M')}"
                )
            if booking.duration:
                hours = int(booking.duration)
                minutes = int((booking.duration - hours) * 60)
                parts.append(f"<strong>Duration:</strong> {hours}h {minutes}m")
            if booking.partner_ids:
                partners = ", ".join(booking.partner_ids.mapped("display_name"))
                parts.append(f"<strong>Attendees:</strong> {partners}")
            booking.ai_narrative = "<br/>".join(parts) if parts else False
