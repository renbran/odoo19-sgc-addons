...
_GROUP_USER = 'sgc_crm_dashboard.group_crm_dashboard_user'
_GROUP_MANAGER = 'sgc_crm_dashboard.group_crm_dashboard_manager'
_GROUP_EXEC = 'sgc_crm_dashboard.group_crm_dashboard_executive'
...
class CrmDashboardController(http.Controller):
    ...
    def dashboard_page(self, **kw):
        ...
        try:
            ...
            return request.render(
                'sgc_crm_dashboard.dashboard_page',
                values,
            )
            ...
            try:
                return request.render(
                    'sgc_crm_dashboard.dashboard_error',
                    values,
                    status=200,
                )
            except Exception:
    ...
    def dashboard_data(self, filter=None, **kw):
        ...
        try:
            ...
            from odoo.addons.sgc_crm_dashboard.services import (
    ...
    def dashboard_refresh(self, **kw):
        ...
        if not _user_has_group(_GROUP_MANAGER):
            raise AccessDenied()
        try:
            from odoo.addons.sgc_crm_dashboard.services import dashboard_cache
    ...
    def dashboard_drilldown(self, drill_type=None, drill_key=None, filter=None, **kw):
        ...
        if not _user_has_group(_GROUP_USER):
            raise AccessDenied()
        try:
            from odoo.addons.sgc_crm_dashboard.services import drilldown
