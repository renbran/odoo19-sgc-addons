...
#
################################################################################
"""Public "big screen" dashboard controller (Cybrosys origin).

Retained from the original CRM Dashboard module so that the public
big-screen page (``/dashboard/big-screen``) keeps working after the
merge with CRM Executive Dashboard. The page is rendered by the
``big_screen_template`` view and refreshes itself from the
``/dashboard/big-screen/data`` JSON endpoint.
"""
