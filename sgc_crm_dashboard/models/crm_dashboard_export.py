...
class CrmDashboardExport(models.TransientModel):
    ...
    def action_generate(self, filter_dict, export_format='csv'):
        ...
        if not is_cron and not self.env.user.has_group(
            'sgc_crm_dashboard.group_crm_dashboard_user'
        ):
            raise AccessError(
    ...
    def _generate_pdf(self):
        ...
        html = self.env['ir.qweb']._render(
            'sgc_crm_dashboard.dashboard_pdf_template',
            values={
                'payload': payload,
                'period': self.period,
    ...
    def _compute_payload(self):
        ...
        if self.env.user.has_group('sgc_crm_dashboard.group_crm_dashboard_manager'):
