...
class CalendarAlarmManager(models.AbstractModel):
    ...
    def _send_reminder(self):
        ...
        all_events_ids = list({
            event_id for event_ids in events_by_alarm.values() for event_id in event_ids
        })
        for alarm_id, event_ids in events_by_alarm.items():
            ...
            events = self.env["calendar.event"].browse(event_ids).with_prefetch(
                all_events_ids)
            events._do_whatsapp_reminder(alarm)
