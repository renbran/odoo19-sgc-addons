...
class Partner(models.Model):
    ...
    def _compute_customer_report_ids(self):
        for partner in self:
            partner.customer_report_ids = self.env['account.move'].search([
                ('partner_id', '=', partner.id),
                ('move_type', '=', 'out_invoice'),
                ('payment_state', '!=', 'paid'),
                ('state', '=', 'posted'),
            ])
    def _get_partner_moves(self, move_type):
    ...
    def main_query(self):
        """Return select query and params"""
    ...
    def amount_query(self):
        """Return query and params for calculating total amount"""
    ...
    def action_share_pdf(self):
        ...
        if self.customer_report_ids:
            main_query, main_params = self.main_query()
            ...
            self.env.cr.execute(main_query, main_params)
    ...
    def auto_week_statement_report(self):
        ...
        for rec in partner:
            if rec.id:
                ...
                self.env.cr.execute(main_query, [self.env.company.id, rec.id])
                main = self.env.cr.dictfetchall()
                data = {
    ...
    def action_vendor_print_pdf(self):
        ...
        if self.vendor_statement_ids:
            ...
            amount_query, amount_params = self.amount_query()
            amount_query += """ AND move_type IN ('in_invoice')"""
