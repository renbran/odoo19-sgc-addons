...
class SGCCopilot(http.Controller):
    ...
    def upload_file(self, **kwargs):
        """Handle file upload, extract text content from PDF / DOCX / TXT."""
        # Rate limit
        remote_ip = request.httprequest.remote_addr or 'unknown'
        if not _check_rate_limit(remote_ip):
            _logger.warning('SGC Copilot upload rate limit from %s', remote_ip)
            return http.Response(
                json.dumps({'result': {'error': 'rate_limited'}}),
        ...
        if 'file' not in http_request.files:
            return http.Response(
                json.dumps({'result': {'error': 'no_file'}}),
                content_type='application/json',
                status=400,
            )

        uploaded = http_request.files['file']
        if not uploaded or not uploaded.filename:
            return http.Response(
                json.dumps({'result': {'error': 'empty_filename'}}),
                content_type='application/json',
                status=400,
            )

        # Validate extension
        allowed = frozenset({'.pdf', '.txt', '.doc', '.docx'})
        filename = uploaded.filename
        ext = os.path.splitext(filename)[1].lower()
        if ext not in allowed:
            return http.Response(
                json.dumps({'result': {'error': 'unsupported_format'}}),
                content_type='application/json',
                status=400,
            )
        ...
        if file_size > 10 * 1024 * 1024:
            return http.Response(
                json.dumps({'result': {'error': 'file_too_large'}}),
                content_type='application/json',
                status=400,
            )
        ...
        try:
            if ext == '.txt':
                ...
                text, page_count = self._parse_pdf(uploaded)

            elif ext in ('.doc', '.docx'):
                text = self._parse_docx(uploaded)

            if len(text) > 50000:
                text = text[:50000] + '\n\n[Content truncated at 50,000 characters]'

            _logger.info(
                'SGC Copilot upload [%s]: %s (%d chars, %d pages)',
                kwargs.get('session_id', 'unknown'), filename, len(text), page_count,
            )
            ...
            _logger.warning('SGC Copilot parse error: %s', exc)
            return http.Response(
                json.dumps({'result': {'error': 'parse_error'}}),
                content_type='application/json',
                status=400,
            )
    ...
    @staticmethod
    def _parse_pdf(file_obj) -> tuple:
        """Extract text from uploaded PDF. Tries PyPDF2 then pdfminer."""
        try:
            import PyPDF2
        ...
        return "PDF text extraction requires PyPDF2 or pdfminer. Please upload a .txt file instead.", 0
    ...
    def _parse_docx(file_obj) -> str:
        ...
        try:
            ...
            return "DOCX text extraction requires python-docx. Please upload a .txt file instead."
